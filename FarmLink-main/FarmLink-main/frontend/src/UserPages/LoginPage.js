...
const LoginPage = ({handleSetProfile}) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // const response = await fetch('http://127.0.0.1:5000/login', {
      const response = await fetch('https://user-management-7c1e7511f413.herokuapp.com/login', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        const profileResponse = await fetch(`https://user-management-7c1e7511f413.herokuapp.com/returnProfile?userId=${data.userId}`);
