...
function Cart({currentUserID}) {
  ...
  const fetchDataForID = async (itemID) => {
    try {
      ...
      const response = await fetch(`https://inventory-catalog-184f236ac862.herokuapp.com/getItem?itemId=${itemID}`);
      if (response.ok) {
  ...
  useEffect(() => {
    const fetchCartData = async (userId) => {
      try {
        ...
        const response = await fetch(`https://shopping-cart-99f9049cebbc.herokuapp.com/returnCart?userId=${userId}`);
        if (response.ok) {
          const data = await response.json();
          console.log(data, "JSON RES");
          // const parsedData = JSON.parse(data);
  ...
  const handleRemoveItemFromCart = (itemId) => {
    const removeFromCart = async (itemId) => { // This function will flush the cart when the user logs out
      try {
        ...
        let response = await fetch("https://shopping-cart-99f9049cebbc.herokuapp.com/removeFromCart", {
          method: "DELETE",
          body: JSON.stringify({
